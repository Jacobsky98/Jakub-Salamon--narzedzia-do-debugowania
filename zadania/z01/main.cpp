/*
  Przy pomocy poznanych narzedzi heaptrack i gdb napraw wszystkie wycieki pamieci w programie

  W docker jest dostepna analiza heaptrack --analyze heaptrack.a.out.*
  Jezeli ktos chce uzywac heaptrack_gui musi zainstalowac sobie dodatkowy pakiet lokalnie (nie bylo mozliwosci umieszczenia go w dockerze z powodu mozliwych problemow z iloscia pamieci)
...
double ro1(double x, double y, double xmax, double ymax, double sigma)
{
  ...
  *result = exp( -1.0*pow(x-0.25*xmax, 2.0)/pow(sigma,2.0) - pow(y-0.5*ymax, 2.0)/pow(sigma, 2.0) );
  return *result;
}
...
{
  double* result = new double;
  *result = -1.0*exp( -1.0*pow(x-0.75*xmax, 2.0)/pow(sigma,2.0) - pow(y-0.5*ymax, 2.0)/pow(sigma, 2.0) );
  return *result;
}

int wypelnienie_macierzy_dirichleta(FILE* wektor, double* a, double* b, int* ja, int* ia, int xmax, int ymax, int sigma, int V1, int V2, int V3, int V4, int nx, int ny, int N, int epsilon1, int epsilon2, bool save_vector) {
...
void metodaPoissonaAlgebraiczna(const char* zad = "")
{
  ...
  int nz_num = wypelnienie_macierzy_dirichleta(wektor, a, b, ja, ia, xmax, ymax, sigma, V1, V2, V3, V4, nx, ny, N, epsilon1, epsilon2, saveVector);
